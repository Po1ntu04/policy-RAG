    ...
    def __init__(self, settings: Settings) -> None:
        ...
        match settings.llm.mode:
            case "llamacpp":
                ...
                model_file_path = models_path / settings.llamacpp.llm_hf_model_file
                if not model_file_path.exists():
                    raise ValueError(
                        "Provided model path does not exist. "
                        f"Expected GGUF model file at: {model_file_path}. "
                        "Fix options: (1) download/copy the .gguf file into the 'models/' folder; "
                        "(2) switch to a different LLM mode/profile (e.g. set PGPT_PROFILES=qwen and provide DASHSCOPE_API_KEY); "
                        "(3) update settings.yaml to use 'openailike' or 'ollama' instead of 'llamacpp'."
                    )
