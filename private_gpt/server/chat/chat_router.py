...
from private_gpt.server.indicators.prompts import SYSTEM_PROMPT_SMART_QA
...
class ChatBody(BaseModel):
    ...
    system_prompt: str | None = Field(
        default=None,
        description="自定义系统提示词，留空则使用默认政务问答提示词"
    )
    use_default_prompt: bool = Field(
        default=True,
        description="是否使用默认政务问答提示词（当system_prompt为空时）"
    )
...
def chat_completion(
    ...
    if not has_system_message:
        # 如果用户未提供system消息，根据配置添加
        if body.system_prompt:
            all_messages.append(
                ChatMessage(content=body.system_prompt, role=MessageRole.SYSTEM)
            )
        elif body.use_default_prompt:
            all_messages.append(
                ChatMessage(content=SYSTEM_PROMPT_SMART_QA, role=MessageRole.SYSTEM)
            )
