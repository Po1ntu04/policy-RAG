...
class IngestService:
    ...
    def _get_base_filename(self, file_name: str) -> str:
        """获取去除后缀的文件名（用于同名检测）"""
        import os
        base = os.path.splitext(file_name)[0]
        return base.lower().strip()

    def check_duplicate_file(self, file_name: str) -> str | None:
        """检查是否已存在同名文件（忽略后缀），返回已存在的文件名或 None"""
        base_name = self._get_base_filename(file_name)
        try:
            existing_docs = self.list_ingested()
            for doc in existing_docs:
                if doc.doc_metadata:
                    existing_file = doc.doc_metadata.get("file_name") or ""
                    if self._get_base_filename(existing_file) == base_name:
                        return existing_file
        except Exception as e:
            logger.warning("检查重复文件时出错: %s", e)
        return None

    def ingest_file(self, file_name: str, file_data: Path) -> list[IngestedDoc]:
        # 检查同名文件（去除后缀比较）
        duplicate = self.check_duplicate_file(file_name)
        if duplicate:
            logger.warning(
                "检测到同名文件已存在: new=%s, existing=%s (仍将继续入库)",
                file_name,
                duplicate,
            )
